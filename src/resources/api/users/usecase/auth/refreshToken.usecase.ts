...
export default class RefreshTokenUsecase implements RefreshToken {
    ...
    async execute(refreshToken: string): Promise<RefreshTokenResponse.Response> {
        try {
            ...
            const newRefreshToken = await this.jwtGenerate.sign((foundUser as any).id, String(process.env.REFRESH_TOKEN_SECRET), '1d')
            const accessToken = await this.jwtGenerate.sign((foundUser as any).id, String(process.env.ACCESS_TOKEN_SECRET), '30s')
